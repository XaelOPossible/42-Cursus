...
int ft_print_arg(va_list args, const char type)
{
    ...
    print_type = 0;
    if (type == 'c')
        print_type += ft_putchar(va_arg (args, char));
    else if (type == 's')
        print_type += ft_printstr(va_arg (args, char *));
    else if (type == 'p')
        print_type += ft_print_hexadecimal(va_arg (args, unsigned long long));
...
int ft_printf(const char *str, ...)
{
    ...
    while (str[i])
    {
        if (str[i] == '%')
        {
            print_len += print_args(args, str[i + 1]);
            i++;
        }
        else
            print_len += ft_printstr(str[i]);
        i++;
    }
    va_end(args);
    return (print_len);
